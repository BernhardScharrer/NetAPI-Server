	...
	void send(int[] data) {
		if (started) {
			if (buffer==data.length) {
				try {
					send_packet = new DatagramPacket(generateIntDatagram(data), BYTE_SIZE*data.length+1, remote, uport);
					socket.send(send_packet);
					System.out.println("Sended datagram.");
				} catch (IOException e) {
					console.warn("Stream broke down!");
					cleanUp();
				}
			} else {
				console.warn("Can't send datagram! (length does not match to: "+data.length+")");
			}
		} else {
